...
if __name__ == '__main__':
	...
	markov1 = [] # list to store memory for current update period   
	...
	for i in range(len(polyVehicles)):
		c = random.uniform(0,1) # flip a weighted coin; returns a random floating point number r such that 0 <= r < 1
		if c < probability1: # i.e. if the vehicle is Switched On...
    ...
    # Memory for last update switched on
			markov1.append(1) # value 1 for switched on
		else:
			markov1.append(0)
	print 'No. of Switched On vehicles at simulation commencement: {}'.format(len(polyVehiclesSwitchedOn))
	print
    
	markov2 = zeroListMaker(len(markov1)) # list to store memory for previous update period   
	...
	while simCounter <= simsRequired:
		...
		print 'Markov1 length: {}'.format(len(markov1))
		print 'Markov2 length: {}'.format(len(markov2))
		print 'polyVehicles length: {}'.format(len(polyVehicles))
		print 'polyVehiclesSwitchedOn length: {}'.format(len(polyVehiclesSwitchedOn))
		...
		while step < endSim: # End simulation if time runs out
			...
			if flag == 0:
				if (step+1000) % update == 0: # evaluates true if and only if 'step+1000' is an exact multiple of 'update'
					...
					markovTemp2 = markov1 # list to store memory for last two update periods periods
					markovTemp = []                 
					...
					for i in range(len(polyVehicles)):
                        ...
                        # 0,0 activated memory comdition
						if (markov1[i] == 0) and (markov2[i] == 0):
							if c < probability1:
								polyVehiclesSwitchedOn.append(polyVehicles[i])
								polyVehiclesSwitchedOnPositions.append(centroid(polyVehicles[i])) # obtain the x and y position of the Switched On Vehicle
								traci.polygon.setColor(polyVehicles[i],(0,102,204,255)) # blue, i.e. Switched On
                  				...
                  				# Memory for last update switched on
								markovTemp.append(1) # value 1 for switched on
							else:
								markovTemp.append(0) 

                        # 0,1 activated memory comdition
						if (markov1[i] == 1) and (markov2[i] == 0):
							if c < probability2:
								polyVehiclesSwitchedOn.append(polyVehicles[i])
								polyVehiclesSwitchedOnPositions.append(centroid(polyVehicles[i])) # obtain the x and y position of the Switched On Vehicle
								traci.polygon.setColor(polyVehicles[i],(0,102,204,255)) # blue, i.e. Switched On
                        ...
                        # 1,0 activated memory comdition
						if (markov1[i] == 0) and (markov2[i] == 1):
							if c < probability3:
								polyVehiclesSwitchedOn.append(polyVehicles[i])
								polyVehiclesSwitchedOnPositions.append(centroid(polyVehicles[i])) # obtain the x and y position of the Switched On Vehicle
								traci.polygon.setColor(polyVehicles[i],(0,102,204,255)) # blue, i.e. Switched On
                        ...
                        # 1,1 activated memory comdition
						if (markov1[i] == 1) and (markov2[i] == 1):
							if c < probability4:
								polyVehiclesSwitchedOn.append(polyVehicles[i])
								polyVehiclesSwitchedOnPositions.append(centroid(polyVehicles[i])) # obtain the x and y position of the Switched On Vehicle
								traci.polygon.setColor(polyVehicles[i],(0,102,204,255)) # blue, i.e. Switched On
		...
		if simCounter <= simsRequired: # i.e. if there are more simulations to do...
			...
			markov1 = [] # list to store memory for last two update periods periods
			...
			markov2 = zeroListMaker(len(markov1))
